...
RangesThreatened <- read_delim("data_revised/Barbarossa/RT_species.csv", 
                 ",",
                 escape_double = FALSE,
                 trim_ws = TRUE,
...
Original_area <- read_delim("data_revised/Barbarossa/species_traits.csv",
                   ",",
                   escape_double = FALSE,
                   trim_ws = TRUE,
...
Areas <- as.data.frame(inner_join(x= RangesThreatened,
                                  y = Original_area,
                                  by = "binomial")) %>%
  # rename because otherwise column name is not recognized when starting with a number
  rename(RT_1.5_ND = '1.5C_no dispersal',
         RT_2.0_ND = '2.0C_no dispersal', 
         RT_3.2_ND = '3.2C_no dispersal',
         RT_4.5_ND = '4.5C_no dispersal', 
         RT_1.5_MD = '1.5C_maximal dispersal',
         RT_2.0_MD = '2.0C_maximal dispersal',
         RT_3.2_MD = '3.2C_maximal dispersal',
         RT_4.5_MD = '4.5C_maximal dispersal',
         Aoriginal = area) %>%
...
EB <- Areas %>% transmute(binomial = binomial,
                          EB_fraction_1.5_ND = 1 - (Anew_1.5_ND / Aoriginal)^z, 
...
# Average species extinction risk across scenarios
Single_species <- EB %>% rowwise() %>% transmute(binomial = binomial,
                                                 EB_fraction_av = mean(c(EB_fraction_1.5_ND,
                                                      EB_fraction_2.0_ND,
                                                      EB_fraction_3.2_ND,
                                                      EB_fraction_4.5_ND,
                                                      EB_fraction_1.5_MD,
                                                      ...
                                                      EB_fraction_4.5_MD),
                                                      na.rm = TRUE))
species_summary <- summary(Single_species$EB_fraction_av)
...
################################################################################
# Figure 2 - plot of extinction risk vs temperature, marginal vs average type
# of effect factor
...
Fig_2 <- ggplot(data = EB_extinction, aes(x = dT, y = EB_all)) +
...
################################################################################
## Figure 3 - All effect factors

Fig_3 <- ggplot(EF_complete, aes(x = reorder(Index, -EFs), y = EFs)) +
  scale_y_continuous(breaks=seq(0,0.07,0.01), limits = c(0, 0.075)) +
  ...
  theme(text = element_text(size = 10, family = "Helvetica"),
        ...
        axis.text.x = element_text(angle = 0, hjust=1, size=8, family = "Helvetica"),
        legend.position = "bottom",
...
################################################################################
### SAVE FILES
write.csv(EF_complete, file = "~/Desktop/Fish_CF_codedata_revised/Effect_factors/EFs.csv")
write.csv(Single_species, file = "~/Desktop/Fish_CF_code/data_revised/Single_species/Extinction_species.csv")
ggsave(path = "visualisations_revised/", filename = "Fig_2.tiff", Fig_2, dpi = 500, width = 90/25.4, height = 90/25.4, unit = 'in')
ggsave(path = "visualisations_revised/", filename = "Fig_3.tiff", Fig_3, width = 190/25.4, height = 100/25.4, unit = 'in', dpi = 500)
